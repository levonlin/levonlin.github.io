...
在我们的示例里，一个文本框中用了该属性的默认值`text`，该值表示一个基本的单行文本框，用于接收无控制或验证的任何文本。而另外一个文本框则使用了值`email`，表示定义一个只接收合法邮箱地址单行文本框，也就是说它会对用户输入的数据进行一些检查。我们会在[表单数据校验]()这篇文章里讨论更多和表单验证有关的内容，
